<script lang="ts">
	...
	let circles = $derived(
		[...Array(circle.count).keys()].map((index) => ({
			...
			offset: (index * 1000) / (circle.count - 1)
		}))
	);

	const lightness = (mode: 'light' | 'dark', target: { lightness: number }) => {
		if (mode === 'light') {
			return target.lightness;
		} else if (mode === 'dark') {
			return 1 - target.lightness;
		}
	};
</script>
...
{#snippet logo(mode: 'light' | 'dark')}
	<svg viewBox="0 0 1000 1000" version="1.1" id="svg1" xmlns="http://www.w3.org/2000/svg">
		{#each circles as { hue, offset }}
			<circle
				style="fill: oklch({circle.lightness[mode]} {circle.chroma} {hue} / {circle.alpha})"
				cx={offset}
				cy="500"
				r={circle.radius}
			/>
		...
		{#if background.visible}
			<path
				id="bg"
				style="fill: oklch({background.lightness[mode]} {background.chroma} {circles.at(-1)?.hue})"
...
{#snippet param(
	...
	{ min = 0, max = 1, step = 0.01 }: { min: number; max: number; step: number }
)}
	<label>
		<input type="range" {oninput} {min} {max} {step} />
		{label}
	</label>
{/snippet}
...
<main>
	...
	<menu>
		...
		<h1>circles</h1>
		<label>
			<input type="range" bind:value={circle.count} min={5} max={10} />
			count
			{circle.count}
		</label>
		<label>
			<input type="range" bind:value={circle.lightness.light} min={0} max={1} step="0.01" />
			lightness (light)
			{circle.lightness.light}
		</label>
		<label>
			<input type="range" bind:value={circle.lightness.dark} min={0.5} max={0.7} step="0.01" />
			lightness (dark)
			{circle.lightness.dark}
		</label>
		<label>
			<input type="range" bind:value={circle.chroma} min={0.05} max={0.2} step="0.001" />
			chroma
			{circle.chroma}
		</label>
		<h1>background</h1>
		...
		<label>
			<input type="range" bind:value={background.lightness.light} min={0.8} max={1} step="0.01" />
			lightness (light)
			{background.lightness.light}
		</label>
		<label>
			<input type="range" bind:value={background.lightness.dark} min={0} max={0.4} step="0.01" />
			lightness (dark)
			{background.lightness.dark}
		</label>
		<label>
			<input type="range" bind:value={background.chroma} min={0} max={0.1} step="0.001" />
			chroma
			{background.chroma}
		</label>
	</menu>
